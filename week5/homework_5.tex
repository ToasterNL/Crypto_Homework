...
We learned that the characteristic polynomial to reduce with cannot simply be determined for non-prime fields, but that we can start our tries with $x^n + ax + b$ for fields of size $p^n$.
...
x^4 + x + 1
sage: F.<x> = GF(2)[]
sage: R=GF(2^4,name='g', modulus=x^4+x+1, check_irreducible=True)
\end{verbatim}
...
The secret for the public key (118, 857) is the modular inverse of 857 modulo 118, which is 99.
