...
Since we are working with a multiplicative (non-)field of characteristic $2$, $\frac{1}{x}$ should then be interpreted as the multiplicative inverse of $x$ modulo $2$.
...
Every $n$ is coprime to 2 and thus an inverse exists, and thus we can always find a non-zero element that gives zero when multiplied by $x$, and thus $\mathbb{R}$ is not a field.
